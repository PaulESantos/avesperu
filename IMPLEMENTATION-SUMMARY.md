...
Se han implementado mejoras completas en el paquete `avesperu` siguiendo
las mejores prácticas de testthat 3+ según la skill
`/revicion-r-packages`, junto con correcciones robustas para el
...
**Título:** “Implement comprehensive testthat 3+ improvements for
avesperu package”
...
**Título:** “Implement robust parallel processing with fallback and
environment-aware core capping”
...
| create\_\*\_result() | 18      | Funciones helper                  |
...
**Problema:** Un único archivo `test-test_search_avesperu.R` con nombre
redundante **Solución:** - Reorganizado en 6 archivos por función -
Sigue estándar testthat: `R/function.R` →
`tests/testthat/test-function.R` - Más fácil navegar y mantener
...
`find_duplicates()`, helpers sin tests **Solución:** - Archivos
dedicados para cada función interna - 19 tests para standardize_names -
11 tests para find_duplicates - 18 tests para create\_\*\_result

### 3. Tests de Paralelismo Fallaban en Checks

**Problema:** Error “19 simultaneous processes spawned” **Causa:**
`detectCores() - 1` intentaba crear 19+ cores en entornos restrictivos
**Solución:** - Cálculo seguro:
`min(detectCores, batches, mc.cores, 4L)` - Fallback automático si el
cluster falla - Tests usan `n_cores = 2L` o
[`withr::local_options()`](https://withr.r-lib.org/reference/with_options.html)
...
✅ **Edge Case Coverage** - Empty input: `character(0)` - NA values:
`c("species", NA)` - Empty strings: `c("")` - Parameter validation -
...
✅ **Fallback Automático** - Error en cluster creation → warning +
secuencial - Resultado siempre correcto - No hay fallos silenciosos
...
Reorganización de archivos de tests

Conversión a sintaxis BDD

Tests para funciones internas

Tests para edge cases

Helper expectations file

Tests para paralelismo (actualizado)

Cálculo seguro de n_cores

Fallback automático

Respeto de getOption(“mc.cores”)

Capping prudente (máximo 4 cores)
...
1.  `parallel = TRUE` es seguro (probará paralelismo, fallará
    gracefully)
...
**Paquete:** avesperu v0.0.8 **Tests:** 76+ (reorganizados y mejorados)
**Paralelismo:** Robusto y environment-safe **Documentación:** Completa
**CRAN/CI:** Compatible

El paquete está listo para mantenimiento a largo plazo con una suite de
tests moderna, segura y exhaustiva.
